...
class Settings:
  ...
  def load_settings(self, fn):
    ...
    #  Homie, 
    self.homie_device = conf.get('homie_device', "bad_device")
    self.homie_name = conf.get('homie_name', 'Bad Device Dont Use')
    self.mqtt_send_min = conf.get("mqtt_send_min", 15)
    self.nut_ups = conf.get("nut_ups", None)
    # only used to shutdown Hubitat
    self.hubitat_ip = conf.get("hubitat_ip", None)
    self.ups_poll = conf.get("ups_poll", 60)
    self.hubitat_user = conf.get("hubitat_user", "bad_user")
    self.hubitat_pw = conf.get("hubitat_pw", "bad_password")
    self.hubitat_pwr_perc = conf.get("hubitat_pwr_perc", 25)
    self.hubitat_pwr_min = conf.get("hubitat_pwr_min", 10)
